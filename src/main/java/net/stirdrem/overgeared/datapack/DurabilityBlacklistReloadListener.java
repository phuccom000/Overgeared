...
public class DurabilityBlacklistReloadListener extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> resources, ResourceManager resourceManager, ProfilerFiller profiler) {
        DATA.clear();
        System.out.println("Found " + resources.size() + " durability blacklist resources:");
        ...
        for (Map.Entry<ResourceLocation, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                if (jsonElement.isJsonObject()) {
                    ...
                    throw new JsonSyntaxException("Expected JSON object for durability blacklist entry: " + id);
                }
            } catch (Exception e) {
                System.err.println("Failed to parse durability blacklist entry: " + id + ", error: " + e.getMessage());
                e.printStackTrace();
            }
        }
    ...
    private Ingredient parseIngredient(JsonObject json) {
        if (!json.has("item")) {
            throw new JsonSyntaxException("Missing 'item' for durability blacklist entry");
        }
